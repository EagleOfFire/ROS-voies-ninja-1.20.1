...
public class SceauExplosifNiv1Entity extends HangingEntity {
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final EntityDataAccessor<ItemStack> DATA_ITEM = SynchedEntityData.defineId(SceauExplosifNiv1Entity.class, EntityDataSerializers.ITEM_STACK);
    private static final EntityDataAccessor<Integer> DATA_ROTATION = SynchedEntityData.defineId(SceauExplosifNiv1Entity.class, EntityDataSerializers.INT);
    public static final int NUM_ROTATIONS = 8;
    private float dropChance = 1.0F;
    private boolean fixed;

    public SceauExplosifNiv1Entity(EntityType<? extends HangingEntity> pEntityType, Level pLevel) {
        super(pEntityType, pLevel);
    }

    public SceauExplosifNiv1Entity(Level pLevel, BlockPos pPos, Direction pFacingDirection) {
        this(ModEntities.SCEAU_EXPLOSIF_NIV1.get(), pLevel, pPos, pFacingDirection);
    }

    public SceauExplosifNiv1Entity(EntityType<? extends SceauExplosifNiv1Entity> pEntityType, Level pLevel, BlockPos pPos, Direction pDirection) {
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (!this.level().isClientSide) {
            ...
            this.kill();
            this.level().explode(pSource.getEntity(), this.getX(), this.getY(), this.getZ(), Utils.explosionRadius(1), Level.ExplosionInteraction.BLOCK);
        }
    ...
    public SlotAccess getSlot(int pSlot) {
        return pSlot == 0 ? new SlotAccess() {
            public ItemStack get() {
                return SceauExplosifNiv1Entity.this.getItem();
            }

            public boolean set(ItemStack p_149635_) {
                SceauExplosifNiv1Entity.this.setItem(p_149635_);
                return true;
            }
        } : super.getSlot(pSlot);
    }
