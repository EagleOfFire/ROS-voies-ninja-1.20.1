package ros.eagleoffire.rosvoiesninjas.entity.custom.SceauScellement;
...
public class SceauScellementNiv0Entity extends HangingEntity {
   private static final Logger LOGGER = LogUtils.getLogger();
   private static final EntityDataAccessor<ItemStack> DATA_ITEM = SynchedEntityData.defineId(SceauScellementNiv0Entity.class, EntityDataSerializers.ITEM_STACK);
   private static final EntityDataAccessor<Integer> DATA_ROTATION = SynchedEntityData.defineId(SceauScellementNiv0Entity.class, EntityDataSerializers.INT);
   public static final int NUM_ROTATIONS = 8;
   private float dropChance = 1.0F;
   private boolean fixed;

   public SceauScellementNiv0Entity(EntityType<? extends HangingEntity> pEntityType, Level pLevel) {
      super(pEntityType, pLevel);
   }

   public SceauScellementNiv0Entity(Level pLevel, BlockPos pPos, Direction pFacingDirection) {
      this(ModEntities.SCEAU_SCELLEMENT_NIV0.get(), pLevel, pPos, pFacingDirection);
   }

   public SceauScellementNiv0Entity(EntityType<? extends SceauScellementNiv0Entity> pEntityType, Level pLevel, BlockPos pPos, Direction pDirection) {
   ...
   public SlotAccess getSlot(int pSlot) {
      return pSlot == 0 ? new SlotAccess() {
         public ItemStack get() {
            return SceauScellementNiv0Entity.this.getItem();
         }

         public boolean set(ItemStack p_149635_) {
            SceauScellementNiv0Entity.this.setItem(p_149635_);
            return true;
         }
      } : super.getSlot(pSlot);
   }
   ...
   public InteractionResult interact(Player pPlayer, InteractionHand pHand) {
        ...
        if (ClientFuinjutsuData.get_LVL() > 0) {
           DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> ClientHooks.openSceauScellementScreen(pPlayer, this));
        }else {
